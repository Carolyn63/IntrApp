class Sysadmin::AdminprofilesController < Sysadmin::ResourcesController

	def index
		response = IppbxApi.ippbx_get(session[:sys_admin], session[:sys_pass],'sys', 'admin/profile', nil)
		if response.code == '200'
			@profile = ActiveSupport::JSON.decode(response.body)
			@uid = @profile["uid"]
			@loginName = @profile["loginName"]
			@emailId = @profile["emailId"]
			@timezone = @profile["geographicDetails"]["timezone"]
			@locale = @profile["geographicDetails"]["locale"]
			@language = @profile["geographicDetails"]["language"]
			@addressLine1 = @profile["geographicDetails"]["addressLine1"]
			@addressLine2 = @profile["geographicDetails"]["addressLine2"]
			@city = @profile["geographicDetails"]["city"]
			@state = @profile["geographicDetails"]["state"]
			@zipCode = @profile["geographicDetails"]["zipCode"]
			@country = @profile["geographicDetails"]["country"]
		else
			flash[:error]  = t("controllers.get.error.sys_admin_profile") + "<br />" + response.body
		end
		@profile ||= ''
	end

	def update
		action = params[:commit].downcase
		case action
		when "cancel"
			redirect_to :action => :index
		when "change"
			content_json = {"uid" => params[:id].to_i, "password" => params[:change_password][:password], "emailId" => params[:emailId][:id]}.to_json
			response = IppbxApi.ippbx_put(session[:sys_admin], session[:sys_pass],'sys', 'admin/profile', content_json)
			if response.code == '200'
				IppbxNotifier.deliver_sys_admin_self_password_reset(params[:loginName][:name], params[:change_password][:password], params[:emailId][:id]) unless params[:emailId][:id].blank? 
				flash[:notice]  = t("controllers.update.ok.password")
			else
				flash[:error]  = t("controllers.update.error.password") + "<br />" + response.body
			end
			session[:sys_admin] = nil
			session[:sys_pass] = nil
			redirect_to edit_sysadmin_adminprofile_path(params[:id])
		else
			content_json = {
				"uid" => params[:id].to_i,
				"emailId" => params[:edit_profile][:emailId],
				"geographicDetails" => {
					"uid" => params[:geographicDetails][:uid].to_i,
					"timezone" => params[:edit_profile][:timezone],
					"locale" => params[:edit_profile][:locale],
					"language" => params[:edit_profile][:language],
					"addressLine1" => params[:edit_profile][:addressLine1],
					"addressLine2" => params[:edit_profile][:addressLine2],
					"city" => params[:edit_profile][:city],
					"state" => params[:edit_profile][:state],
					"zipCode" => params[:edit_profile][:zipCode],
					"country" => params[:edit_profile][:country]
				}
			}.to_json
			
			logger.info "Putting Json String: #{content_json.to_s}"

			response = IppbxApi.ippbx_put(session[:sys_admin], session[:sys_pass],'sys', 'admin/profile', content_json)

			if response.code == '200'
				flash[:notice]  = t("controllers.update.ok.sys_admin_profile")
				redirect_to :action => :index
			else
				flash[:error]  = t("controllers.update.error.sys_admin_profile") + "<br />" + response.body
				redirect_to :action => :edit
			end
		end
	end

	def edit
		response = IppbxApi.ippbx_get(session[:sys_admin], session[:sys_pass],'sys', 'admin/profile', nil)
		if response.code == '200'
			@profile = ActiveSupport::JSON.decode(response.body)
			@uid = @profile["uid"]
			@loginName = @profile["loginName"]
			@emailId = @profile["emailId"]
			@timezone = @profile["geographicDetails"]["timezone"]
			@locale = @profile["geographicDetails"]["locale"]
			@language = @profile["geographicDetails"]["language"]
			@addressLine1 = @profile["geographicDetails"]["addressLine1"]
			@addressLine2 = @profile["geographicDetails"]["addressLine2"]
			@city = @profile["geographicDetails"]["city"]
			@state = @profile["geographicDetails"]["state"]
			@zipCode = @profile["geographicDetails"]["zipCode"]
			@country = @profile["geographicDetails"]["country"]
			@geographic_details_uid = @profile["geographicDetails"]["uid"]
		else
			flash[:error]  = t("controllers.get.error.sys_admin_profile") + "<br />" + response.body
		end
	end

	def change_password
		render :partial=>"change_password"
	end

end
